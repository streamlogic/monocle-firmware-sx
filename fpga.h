...
/** @file fpga.h
 *	@brief Function prototypes for interface to FPGA.
 *
...
#define FPGA_RELEASE_20210709 // turn on for compatibility with RELEASE_2021-09-07
...
// NOTE taken from monocal_mrb branch; current FPGA does not have these registers; reads will return 0 (& seems won't crash FPGA)
...
//NOTE ^ = this bit triggered by rising edge, write 0 followed by 1 to activate
//Reserved                        bit 7
#define FPGA_CLR_CHKSM            BIT(6) // ^ clear checksum calculation for previously captured frame/video/audio
#define FPGA_RESUME_FILL          BIT(5) // ^ resume video; only effective if CAPT_EN is set
...
#define FPGA_CAPT_EN              BIT(0) // ^ enable capture, use together with 1 or 2 of CAPT_FRM, _VIDEO, _AUDIO
